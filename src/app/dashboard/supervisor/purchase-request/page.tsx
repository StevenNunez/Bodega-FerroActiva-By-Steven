...
const FormSchema = z.object({
  ...
  area: z
    .string()
    .min(3, "El área/proyecto debe tener al menos 3 caracteres."),
});

type FormData = z.infer<typeof FormSchema>;

export default function SupervisorPurchaseRequestPage() {
  const { purchaseRequests, materials, addPurchaseRequest, materialCategories } =
    useAppState();
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Completa el formulario para pedir nuevos materiales. El Jefe de
              Operaciones deberá aprobar la compra.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              El estado de tus solicitudes se actualizará aquí una vez
              gestionadas.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {myRequests.length > pageSize && (
              <div className="flex justify-between items-center mt-4">
                ...
                <span className="text-sm text-muted-foreground">
                  Página {page} de {Math.ceil(myRequests.length / pageSize)}
                </span>
                <Button
                  variant="outline"
                  size="sm"
                  disabled={page >= Math.ceil(myRequests.length / pageSize)}
                  onClick={() =>
                    setPage((p) =>
                      Math.min(p + 1, Math.ceil(myRequests.length / pageSize))
                    )
                  }
                >
                  Siguiente
