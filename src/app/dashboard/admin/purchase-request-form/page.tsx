...
function ReceiveRequestDialog({ request, isOpen, onClose, onConfirm, materials }: ReceiveRequestDialogProps) {
  ...
  React.useEffect(() => {
    if (request) {
      ...
      // Check if a material with the exact name already exists
            ...
            <div className="space-y-2">
                ...
                {existingMaterial ? (
                     <div className="p-3 rounded-md bg-green-100 dark:bg-green-900 border border-green-200 dark:border-green-800">
                        <p className="text-sm font-semibold text-green-800 dark:text-green-200">Material Encontrado</p>
                        <p className="text-xs text-green-700 dark:text-green-300">El stock se añadirá a: <strong>{existingMaterial.name}</strong></p>
                     </div>
                ) : (
                    <Select onValueChange={setSelectedMaterialId} defaultValue={selectedMaterialId}>
                        <SelectTrigger>
                            <SelectValue placeholder="Crear nuevo material o vincular..." />
                        </SelectTrigger>
                        <SelectContent>
                             <SelectItem value="create_new">-- Crear Nuevo Material en Bodega --</SelectItem>
                             {materials.map(m => <SelectItem key={m.id} value={m.id}>{m.name}</SelectItem>)}
                        </SelectContent>
                    </Select>
                )}
                 <p className="text-xs text-muted-foreground">
                    Si el material ya existe, selecciónalo para sumar el stock. Si no, se creará uno nuevo con este nombre.
                </p>
            </div>
        </div>
        <DialogFooter>
...
export default function AdminPurchaseRequestsPage() {
  ...
  const formatDate = (date: Date | Timestamp | null | undefined): string => {
    ...
    const formatDateTime = (date: Date | Timestamp | null | undefined): string => {
    const jsDate = getDate(date);
    return jsDate ? jsDate.toLocaleString('es-CL', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'N/A';
  };
    ...
    <>
      ...
      <div className="flex flex-col gap-8">
        <PageHeader
          ...
          description="Supervisa el estado de todas las solicitudes de compra y registra la recepción de materiales."
        />
        <Card>
          <CardHeader>
