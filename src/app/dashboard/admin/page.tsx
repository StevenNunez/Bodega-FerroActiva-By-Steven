"use client";import { PageHeader } from "@/components/page-header";
...
import { Input } from "@/components/ui/input";type CompatibleMaterialRequest = MaterialRequest & {
    materialId?: string;
    quantity?: number;
};export default function AdminPage() {
  const { requests, tools, toolLogs, users, materials, approveRequest } = useAppState();
  const { toast } = useToast();  const materialMap = React.useMemo(() => new Map(materials.map(m => [m.id, m])), [materials]);
  const pendingRequests = requests.filter(r => r.status === 'pending');
  const checkedOutTools = toolLogs.filter(log => log.returnDate === null);
  const lowStockMaterials = materials.filter(m => m.stock < 100);  const getToolCheckoutInfo = (toolId: string) => {
  ...
  return (
    <div className="flex flex-col gap-8">
      <PageHeader title="Resumen de Administrador" description="Vista general del estado de la bodega y acciones rápidas." />  <ToolCheckoutCard />
        ...
        <Card>
         ...
         <Card>
            ...
            <CardContent className="space-y-4">
                ...
                />
                 <ScrollArea className="h-96 whitespace-nowrap border rounded-md">
                        ...
                        <Table className="min-w-[600px]">
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="w-[200px] text-right">En posesión de</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {filteredTools.map(tool => {
                                    ...
                                    <TableRow key={tool.id}>
                                        <TableCell className="font-medium truncate">{tool.name}</TableCell>
                                        <TableCell>
                                            <Badge 
