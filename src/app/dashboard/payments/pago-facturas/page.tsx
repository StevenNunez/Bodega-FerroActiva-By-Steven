...
const CreatePaymentForm = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!invoiceNumber || !supplierId || !amount || !dueDate || !work) {
      toast({
        variant: "destructive",
        title: "Error",
...
// === Badge de estado ===
const PaymentStatusBadge = ({ status, payment }: { status: PaymentStatus, payment: SupplierPayment }) => {
  switch (status) {
    case "pending":
      return (
...
export default function PaymentManagementPage() {
  ...
  const handleMarkAsPaid = async (details: { paymentDate: Date; paymentMethod: string }) => {
    ...
    await updateSupplierPaymentStatus(payingPayment.id, "paid", details);
