...
const ApprovedRequestsCard = memo(
  ({
    ...
    handleArchive: (requestId: string) => Promise<void>;
  }) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          <AlertDialogTitle>¿Archivar Solicitud?</AlertDialogTitle>
                          <AlertDialogDescription>
                            Esta acción marcará la solicitud para "{req.materialName}" como gestionada y la quitará de esta lista. No se eliminará del historial. ¿Estás seguro?
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
...
const ActiveLotsCard = memo(
  ({ batchedLots, handleRemove }: { batchedLots: Lot[]; handleRemove: (requestId: string) => Promise<void> }) => (
    <Card className="transition-all duration-300">
      <CardHeader>
  ...
  const handleArchive = useCallback(
    async (requestId: string) => {
      if (!requestId) {
       ...
       setIsLoadingAction(requestId);
      try {
        await updatePurchaseRequestStatus(requestId, 'ordered', { notes: 'Gestionada y archivada desde panel de lotes.' });
        toast({ title: "Solicitud Archivada", description: "La solicitud ha sido marcada como gestionada y quitada de la lista.", variant: 'default' });
      } catch (e: any) {
        toast({
          variant: "destructive",
          title: "Error",
          description: e?.message || "Error inesperado al archivar la solicitud.",
        });
      } finally {
        setIsLoadingAction(null);
      }
    ...
    <div className="flex flex-col gap-8 p-6 bg-background min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-8">
          <CreateLotCard />
          <ActiveLotsCard batchedLots={batchedLots} handleRemove={handleRemove} />
        </div>
      </div>
    </div>
  );
