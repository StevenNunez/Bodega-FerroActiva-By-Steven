                        ...
                        {sortedObservations.length > 0 ? (
                            <div className="space-y-3 p-4">
                                {sortedObservations.map(obs => (
                                    <Link key={obs.id} href={`/dashboard/safety/review-observations/${obs.id}`} passHref>
                                        <div className="p-4 border rounded-lg flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 hover:bg-muted/50 transition-colors cursor-pointer">
                                            <div className="flex-grow">
                                                ...
                                                <p className="text-xs text-muted-foreground mt-1">Registrado por: {obs.observerName} el {formatDate(obs.createdAt)}</p>
                                            </div>
