...
function addHeader(doc: jsPDF, inspection: SafetyInspection, logo?: string) {
  ...
  doc.setFontSize(11);
  doc.setTextColor(COLORS.text);
  doc.text(`Obra: ${inspection.work}`, pageWidth / 2, 28, { align: 'center' });
...
async function addSignatures(doc: jsPDF, y: number, inspection: SafetyInspection, supervisor: User, apr: User) {
  ...
  await addSignature('Firma Ejecutor (Cierre)', inspection.completionSignature, supervisor.name, inspection.completedAt, leftX);
  await addSignature('Firma Revisor (APR)', inspection.reviewedBy?.signature, apr.name, inspection.reviewedBy?.date, rightX);
}
...
export async function generateInspectionPDF(inspection: SafetyInspection, supervisor: User, apr: User) {
  ...
  const filename = `Inspeccion_${inspection.work.replace(/[^a-zA-Z0-9]/g, '_')}_${formatDate(inspection.createdAt)}.pdf`;
  doc.save(filename);
}
