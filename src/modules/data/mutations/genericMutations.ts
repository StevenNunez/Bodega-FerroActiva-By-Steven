...
export async function addMaterial(data: any, { user, tenantId, db }: Context) {
    ...
    const materialColl = collection(db, `tenants/${tenantId}/materials`);
    ...
    await runTransaction(db, async (transaction) => {
        ...
        if (data.stock > 0) {
            const movementRef = doc(collection(db, `tenants/${tenantId}/stockMovements`));
...
export async function updateMaterial(materialId: string, data: any, { tenantId, db }: Context) {
    if (!tenantId) throw new Error("Inquilino no válido.");
    const materialRef = doc(db, `tenants/${tenantId}/materials`, materialId);
    await updateDoc(materialRef, data);
}

export async function deleteMaterial(materialId: string, { tenantId, db }: Context) {
    if (!tenantId) throw new Error("Inquilino no válido.");
    await deleteDoc(doc(db, `tenants/${tenantId}/materials`, materialId));
}
...
export async function addMaterialCategory(name: string, { tenantId, db }: Context) {
    if (!tenantId) throw new Error("Inquilino no válido.");
    await addDoc(collection(db, `tenants/${tenantId}/materialCategories`), { name, tenantId });
}

export async function updateMaterialCategory(id: string, name: string, { tenantId, db }: Context) {
    if (!tenantId) throw new Error("Inquilino no válido.");
    await updateDoc(doc(db, `tenants/${tenantId}/materialCategories`, id), { name });
}

export async function deleteMaterialCategory(id: string, { tenantId, db }: Context) {
    if (!tenantId) throw new Error("Inquilino no válido.");
    await deleteDoc(doc(db, `tenants/${tenantId}/materialCategories`, id));
}

export async function addUnit(name: string, { tenantId, db }: Context) {
    if (!tenantId) throw new Error("Inquilino no válido.");
    await addDoc(collection(db, `tenants/${tenantId}/units`), { name, tenantId });
}

export async function deleteUnit(id: string, { tenantId, db }: Context) {
    if (!tenantId) throw new Error("Inquilino no válido.");
    await deleteDoc(doc(db, `tenants/${tenantId}/units`, id));
}
...
export async function addSupplier(data: any, { tenantId, db }: Context) {
    if (!tenantId) throw new Error("Inquilino no válido.");
    await addDoc(collection(db, `tenants/${tenantId}/suppliers`), { ...data, tenantId });
}
...
export async function createLot(name: string, { user, tenantId, db }: Context) {
    if (!user || !tenantId) throw new Error("No autenticado o sin inquilino.");
    const lotRef = collection(db, `tenants/${tenantId}/purchaseLots`);
    await addDoc(lotRef, {
        name,
...
export async function addRequestToLot(requestId: string, lotId: string, { tenantId, db }: Context) {
    if (!tenantId) throw new Error("Inquilino no válido.");
    const requestRef = doc(db, `tenants/${tenantId}/purchaseRequests`, requestId);
...
export async function deleteLot(lotId: string, { tenantId, db }: Context) {
    ...
    const requestsQuery = query(collection(db, `tenants/${tenantId}/purchaseRequests`), where('lotId', '==', lotId));
    ...
    // Delete the lot
    const lotRef = doc(db, `tenants/${tenantId}/purchaseLots`, lotId);
    batch.delete(lotRef);
...
// --- Permissions ---
export async function updateRolePermissions(role: any, permission: any, checked: any, { tenantId, db }: Context) {
  if (!tenantId) throw new Error("Inquilino no válido.");
